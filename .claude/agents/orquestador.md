...
- **Modo loop (semi-automático):** para una unidad de trabajo acotada y repetitiva podés correr el protocolo [`/loop-cerrado`](../../.claude/commands/loop-cerrado.md): aislás en un worktree (`scripts/loop.*`), iterás (implementador → revisor → auditor-seguridad si es crítico) con **tope de 4 vueltas** y registrás el avance en [`loops/`](../../loops/). No cruza a producción solo; en zona crítica frena para el OK humano antes de `main`.
